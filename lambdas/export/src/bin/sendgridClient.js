...
const maskError = require('./maskError').default;
...
module.exports = {
  ...
  send: async (email) => {
    ...
    if (email.pathToAttachment) {
      try {
        const attachment = fs.readFileSync(email.pathToAttachment).toString("base64");
        email.attachments = [
          {
            content: attachment,
    ...
    try {
      ...
      const error = maskError(e, "Ocurrió un error al enviar el mail, revise que este bien configurado.");
      console.error(e);
      throw error;
    }
    /* Inserting the email into a mongoDB collection. */
    return mongodb.collection('historialemails').insert({...email, fecha: moment.utc().toDate()})
  ...
  format: ({ template, asunto, destinatario, from, cc, bcc }, parametros) => {
 ...
 */
function removerDuplicados(email) {
  ...
  const duplicadosBCC = _.intersection(email.to, email.bcc).concat(_.intersection(email.cc, email.bcc));
